...
from torch.utils.data import default_collate as torch_collate
...
def _wds_pipeline(tar_pattern, transform, *, shuffle=10000, shardshuffle=1000, batch_size=256):
    shards = sorted(glob.glob(tar_pattern))
    if len(shards) == 0:
        raise FileNotFoundError(f"No shards match pattern: {tar_pattern}")
    ds = (
        wds.WebDataset(shards, shardshuffle=shardshuffle)
        .shuffle(shuffle)
        .decode("pil")
        .to_tuple("jpg;png;jpeg", "cls")
        .map_tuple(lambda img: transform(img), _cast_label)  # transform MUST return a Tensor
    )
    return ds
...
def _find_latest_checkpoint(ckpt_dir: Path) -> Path | None:
    # prefer numbered epoch checkpoints
    cands = sorted(ckpt_dir.glob("checkpoint_epoch_*.pth"))
    if cands:
        # pick highest epoch number
        cands.sort(key=lambda p: int(p.stem.split("_")[-1]))
        return cands[-1]
    # fallback to best model (if you want)
...
def _cast_label(y):
    if isinstance(y, (bytes, bytearray)):
        y = y.decode("utf-8")
    if hasattr(y, "item"):  # numpy scalars / torch tensors
        y = y.item()
    return int(y)
...
class Trainer:
    def __init__(self, cfg: dict):
        ...
        # Model
        self.model = resnet50(weights=None, num_classes=1000)  # from scratch
        ...
        try:
            ...
            if latest is not None:
                ...
                # resume epoch (next epoch to run)
                self.epoch = int(state.get("epoch", 0))
                # align LR scheduler global step (t counts iterations)
                self.lr_sched.t = (self.epoch) * len(self.train_loader)
                # restore best acc if present
                self.best_acc1 = float(state.get("best_acc1", 0.0))
                logger.info(f"Resumed from {latest.name} at epoch {self.epoch+1} "
                            f"(best acc1={self.best_acc1:.2f}).")
            else:
                logger.info("No checkpoint found; starting fresh.")
        except Exception as e:
            logger.warning(f"Auto-resume skipped due to error: {e}; starting fresh.")
        Path(self.cfg["checkpoint_dir"]).mkdir(parents=True, exist_ok=True)
        self.mlogger = MetricsLogger(self.cfg["checkpoint_dir"])
    ...
    def _build_dataloaders(self):
        ...
        if self.cfg.get("use_randaugment", False):
        ...
        # guard prefetch_factor when single-process
        pf = self.cfg["prefetch_factor"] if self.cfg["num_workers"] > 0 else None

        if self.cfg.get("data_format", "folder") == "wds":
            # -------- WebDataset path --------
            if wds is None:
                raise RuntimeError("webdataset not installed. pip install webdataset")
            data_dir = Path(self.cfg["data_dir"])
            train_pat = str(data_dir / self.cfg["train_pattern"])
            val_pat   = str(data_dir / self.cfg["val_pattern"])
            
            
            shuf = self.cfg.get("wds_shardshuffle", 1000)
            bs   = self.cfg["batch_size"]

            train_set = _wds_pipeline(
                train_pat, train_tfms, shuffle=10000, shardshuffle=shuf, batch_size=bs
            ).with_length(self.cfg["samples_per_epoch"])

            val_set = _wds_pipeline(
                val_pat,   val_tfms,   shuffle=0,     shardshuffle=shuf, batch_size=bs
            ).with_length(self.cfg["val_samples"])

            # IMPORTANT: batching is already done inside WebDataset → set batch_size=None
            pf = self.cfg["prefetch_factor"] if self.cfg["num_workers"] > 0 else None
# IMPORTANT: batch_size=None and drop_last=False when batching in WDS
            ...
            train_set = ImageFolder(str(train_dir), transform=train_tfms)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, epochs):
            ...
            self.mlogger.log_epoch(
                epoch=epoch + 1,
                train_loss=train_stats["loss"],
                val_loss=val_stats["loss"],
                acc1=val_stats["acc1"],
                acc5=val_stats["acc5"],
                lr=self.lr_sched.get_lr(),
            )
    ...
    def _train_one_epoch(self):
        ...
        for i, (images, target) in enumerate(self.train_loader):
            ...
            with autocast(enabled=self.cfg["use_amp"], device_type=self.amp_device_type):
